...
from cv2.legacy import TrackerKCF_create
...
YOLOV3_CFG = 'D:/HK2_Nam_3\Xu_Ly_Anh_So/project_xlas/vehicles_counting-master/yolov3-tiny.txt'
YOLOV3_WEIGHT = 'D:/HK2_Nam_3/Xu_Ly_Anh_So/project_xlas/vehicles_counting-master/yolov3-tiny.weights'
...
def show():
    st.title("Nhận diện và đếm số lượng xe")
    st.markdown("""
    <div style='font-size:18px; color:#43c6ac;'>
        Ứng dụng sử dụng YOLO để nhận diện và đếm số lượng xe trong video.<br>
        Bạn có thể upload video hoặc sử dụng video mẫu có sẵn.
    </div>
    """, unsafe_allow_html=True)
    
    video_file = st.file_uploader("Tải lên video (mp4, avi, mov)", type=["mp4", "avi", "mov"])
    use_sample = st.checkbox("Hoặc sử dụng video mẫu", value=True)
    
    if use_sample:
        video_input = 'D:/HK2_Nam_3/Xu_Ly_Anh_So/project_xlas/vehicles_counting-master/highway.mp4'
    elif video_file is not None:
        with open("uploaded_video.mp4", "wb") as f:
            f.write(video_file.read())
        video_input = "uploaded_video.mp4"
    else:
        st.info("Vui lòng upload video hoặc chọn video mẫu.")
        return
    
    video_output = "output_counting.avi"
    if st.button("Bắt đầu nhận diện và đếm xe"):
        counting_vehicle(video_input, video_output)
        st.success("Hoàn thành nhận diện và đếm xe!")
